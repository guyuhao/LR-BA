"""
evaluate LR-BA under gradient compression protection with different gradient compression percent
"""
...
gc_percent_list = [0.2, 0.4, 0.6, 0.8, 0.9]
...
def run_compare_experiments(train_loader, test_loader, backdoor_train_loader, backdoor_test_loader, args,
    ...
    args['gradient_compression'] = True
    for i in range(len(gc_percent_list)):
        gc_percent = gc_percent_list[i]
        # evaluate LR-BA with current gradient compression percent
        logging.info('------- gradient compression percent: {} -------'.format(gc_percent))
        args['gc_percent'] = gc_percent

        vfl = get_vfl(args=args, backdoor_indices=backdoor_indices)
